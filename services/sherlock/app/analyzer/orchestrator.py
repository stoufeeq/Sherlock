...
from app.analyzer import (
    api_gateway, asyncapi, code_refs, files, manifests, openapi, sql,
...
def scan_repo(*, app_name: str, clone_url: str, ref: str = "main") -> AnalysisResult:
    ...
    try:
        ...
        return scan_directory(app_name=app_name, root=tmp, commit_sha=sha)
    finally:
        shutil.rmtree(tmp, ignore_errors=True)


def scan_directory(*, app_name: str, root: Path, commit_sha: str = "") -> AnalysisResult:
    """Run all analyzers against an already-on-disk working tree.

    Used both by `scan_repo` (after cloning) and by `/analyze-diff` (after
    overlaying working-tree files onto a base clone). Side-effect-free besides
    walking `root` and reading files.
    """
    result = AnalysisResult(app_name=app_name, commit_sha=commit_sha)
    ...
    # Post-process: resolve API-gateway URLs to backend apps. Mutates
    # `result.called_endpoints` in place and records gateway provenance in
    # `result.gateway_resolved`.
    rewritten = api_gateway.resolve_in_place(result)
    if rewritten:
        log.info("api gateway resolver rewrote %s call(s) for %s", rewritten, app_name)
